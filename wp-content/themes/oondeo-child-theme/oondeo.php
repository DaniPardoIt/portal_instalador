...
function procesar_formulario( $contact_form, &$abort, $submission ){
	...
	document_info('procesar_formulario.txt', 'contact_form->name()', $contact_form->name(), true);

	
	if( $contact_form->name() == 'registro' ){
		$user_uri = WP_PLUGIN_DIR."/aa-oondeo-functions/UserController/UserRouter.php";
		document_info('procesar_formulario.txt', 'Plugin URI',$user_uri, true);
		
		require_once $user_uri;
		register_form( $datos_formulario );
		return;
	}

	$uploaded_files = $submission->uploaded_files();
	$form_action =  $datos_formulario['form_action'];
	$post_id = null;
	
	if( $oondeo_config['log_level'] >= 2 ){
		// document_info('procesar_formulario.txt', "contact_form", $contact_form, true);
		document_info('procesar_formulario.txt', '$datos_formulario: ', $datos_formulario, true);
		document_info('procesar_formulario.txt', '$uploaded_files: ', $uploaded_files, true);
		document_info('procesar_formulario.txt', '$form_action: ', $form_action, true);
	}
	...
	// }else{
	// 	onsend_solicitud( $datos_formulario, $uploaded_files );
	// }

	switch( $form_action ){
		case "save":
			if( isset($datos_formulario['post_id']) && !empty($datos_formulario['post_id']) ){
				$post = get_full_post( array( 'post_type' => 'solicitud', 'p' => $datos_formulario["post_id"] ) );
				$permitted = apply_filters( 'check_solicitud_permission', 'edit', $post );
					
			}else{
				$permitted = apply_filters( 'check_solicitud_permission','solicitud_create' );
			}
		...
		case "send":
			...
			$post_id = onsave_solicitud( $datos_formulario, $uploaded_files );

			document_info('procesar_formulario.txt', 'switch(form_action) -> case="send" ', array( "post_id" => $post_id, "datos_formulario"=> $datos_formulario, "uploaded_files" => $uploaded_files ) , true);

			onsend_solicitud( $post_id );
			break;

		case "remove":
			if( $post_id ){
				remove_solicitud( $post_id );
			}else{
				return false;
			}
...
function onsave_solicitud( $data, $uploaded_files ){
	...
	//Borrar campos del formulario que solo utilizamos para el frontend
 	$data = array_remove_keys( $data, array('save_button', 'opcion', 'hay*', 'form_action') );
	if( $oondeo_config['log_level'] >= 2 ){
	...
	if( !isset($data['post_id']) || isset($data['post_id']) && empty($data['post_id']) ){
		return new_solicitud( $data, $uploaded_files );
	}else{
		return existent_solicitud( $data, $uploaded_files );
	}
}
...
function existent_solicitud( $data, $uploaded_files ){
	...
	if( in_array( $data['estado_solicitud'], array('error_oca') ) ){
		$data['estado_solicitud'] = 'creacion';
	}
...
function get_term_abbreviatons( $taxonomies=array('categoria', 'tipo_solicitud', 'tipo_expediente') ){
	
	$info_path = __DIR__.'/'.basename(__FILE__, '.php').'->get_term_abbreviations.txt';
	document_info( $info_path, 'Search Solicitudes', '' );
	
	global $wpdb;
	$results = $wpdb->get_results( "SELECT tm.meta_key, tm.meta_value, tt.taxonomy, t.slug, t.name FROM `wp_terms` AS `t` JOIN `wp_term_taxonomy` AS `tt` ON t.term_id = tt.term_id JOIN `wp_termmeta` AS `tm` ON t.term_id = tm.term_id WHERE tt.taxonomy IN ('categoria', 'tipo_solicitud') AND tm.meta_key = 'short_name'", ARRAY_A );

	document_info( $info_path, 'Resultssss', $results );
}

function get_solicitudes_abiertas_count(){
	
	$info_path = __DIR__.'/'.basename(__FILE__, '.php').'->'.__FUNCTION__.'.txt';
	
	global $wpdb;
	$results = $wpdb->get_results( "SELECT count(p.ID) FROM wp_posts AS p JOIN wp_postmeta AS pm ON pm.post_id = p.ID WHERE p.post_type = 'solicitud' AND post_status = 'publish' AND pm.meta_key = 'estado_solicitud' AND pm.meta_value != 'finalizada'", ARRAY_N );

	$num_solicitudes = $results[0][0];

	document_info( $info_path, 'Solicitudes Abiertas', array(
		'result'					=>	$results,
...
function get_solicitudes_cerradas_count(){
	...
	document_info( $info_path, 'Solicitudes Cerradas', array(
		'result'					=>	$results,
		'num_solicitudes'	=>	$num_solicitudes
	) );
...
function onsend_solicitud( $post_id ){
	global $oondeo_config, $user;

	document_info('onsend_solicitud.txt', "function onsend_solicitud(post_id: $post_id)", '');

	$post = get_full_post( array(
		'post_type'			=>	'solicitud',
		'p'						=>	$post_id
	) );
	$post->meta_data = remove_unnecessary_nested_array( $post->meta_data);

	document_info('onsend_solicitud.txt', "post", $post, true);
	
	if( $oondeo_config['log_level'] >= 2 ){
		
	}

	$estado_solicitud = $post->meta_data['estado_solicitud'];
	$pod = pods('solicitud', $post_id);

	document_info('onsend_solicitud.txt', "estado_solicitud: $estado_solicitud ", '', true);

	if( in_array( $estado_solicitud, array('creacion', 'error_oca') ) ){
		$post->meta_data['estado_solicitud'] = 'revision_oca';
		$pod->save('estado_solicitud', 'revision_oca');
		document_info('onsend_solicitud.txt', "POST SAVED(creacion|error_oca) ", $post, true);

	}else if( in_array( $estado_solicitud, array('revision_oca') ) ){
		
		// Juanra, aquí va la llamada al exim. Tienes el $post que contiene toda la información que necesitas. Puedes utilizar esta función para obtener un JSON con los campos del post ya simplificados:
		// post_to_JSON( $post );

		if ( function_exists( 'exim_rest_post' ) ) {
			document_info('onsend_solicitud.txt', "function_exists('exim_rest_post') ", 'true', true);
			if( exim_rest_post($post) ){
				$post->meta_data['estado_solicitud'] = 'enviada';	
				$pod->save('estado_solicitud', 'enviada');
			}

		}else{
			document_info('onsend_solicitud.txt', "function_exists('exim_rest_post') ", 'false', true);
		}
		
		// Dependiendo lo que devuelva el EXIM hay que poner el estado del post en uno de los siguientes valores: error_exim | finalizada:
		// $pod = pods('solicitud', $post_id);
		// $pod->save('estado_solicitud', 'error_exim'); //Error
		// $pod->save('estado_solicitud', 'finalizada'); //OK
	}
}
...
function get_full_posts( $args ){
	...
	document_info( 'get_full_posts.txt', 'POSTS', $posts, true);
	return $posts;
}

/**
 * @param 	$field		String			The field to retrieve the user with. id | ID | slug | email | login.
 * @param		$value		String|int	A value for $field. A user ID, slug, email address, or login name.
 * 
 * @return WP_User|false	WP_User object on success, false on failure.
 */
function get_full_user( $field, $value ){
	$user = get_user_by( $field, $value );
	$user->meta_data = get_user_meta( $user->ID );
	document_info('get_full_user.txt', 'User', $user);
...
function simplify_post( $post ){
	
}

function posts_to_JSON( $posts ){
	document_info('posts_to_JSON.txt', 'posts', $posts);
	$posts_JSON = [];
	foreach( $posts as $post ){
		$posts_JSON[] = json_decode( post_to_JSON( $post ) );
	}

	document_info('posts_to_JSON.txt', 'posts_JSON', $posts_JSON, true);
	return json_encode($posts_JSON);
}
...
function _oondeo_check_permission($action,$post=null){
	...
	document_info( 'oondeo_check_permission.txt', "", "" );
	$permitted = false;
	...
	if( !empty($post) ){
		...
		if( $oondeo_config['log_level'] >= 2 ){
			document_info( 'oondeo_check_permission.txt', "Own Solicitud", $own_solicitud, true );
		}
	}
	...
	switch ($action) {
		...
		case "create":
			$permitted = user_can( $user_id, 'solicitud_create' );
			break;

		case "error":
			$permitted = user_can( $user_id, 'solicitud_error_management' );
			break;

		case "send":
			$permitted = user_can( $user_id, 'solicitud_send' );
			break;

		case "send_exim":
			$permitted = user_can( $user_id, 'solicitud_send_exim' );
			break;
	...
	//If user is not allowed or if there is no post return false
	if( !$permitted || empty($post) ){
		return $permitted;
	}
...
function htmlToJson($html) {
	...
	document_info( $info_path, '$html:', $html);

	// convert html to array using DOMDocument
  $dom = new DOMDocument();
  $dom->loadHTML($html);
  $elements = $dom->getElementsByTagName("*");
  $array = array();
  foreach ($elements as $element) {
		document_info( $info_path, "element:", $element, true);
    $nodeArray = array();
    $nodeArray['tag'] = $element->tagName;
    $nodeArray['attributes'] = array();
    foreach ($element->attributes as $attribute) {
      $nodeArray['attributes'][$attribute->nodeName] = $attribute->nodeValue;
    }
    $nodeArray['content'] = $element->nodeValue;
  ...
  $json = json_encode($array);
	...
	document_info( $info_path, '$doc:', $array, true);
	document_info( $info_path, '$json:', $json, true);
	
	// Return the JSON string
	return $json;
}
htmlToJson("<div class='pepinillo' data-info='durum'><p>Pero si resulta que lo más peligroso de todo esto va a <b>ser que no</b></p>
							<span>Oondeo <strong data-input='loquequieras.png'>Repeater</strong></span>
						</div>");
/* FIN HTML TO JSON */
...
// 	if ( has_filter( 'check_solicitud_permission' ) ) {
// 		$permitted = apply_filters( 'check_solicitud_permission', 'edit', $post );
// 		document_info( 'prueba.txt', 'Filter', $permitted );
// 	}
// 	document_info( 'prueba.txt', 'Post', $post, true );
// }
// /* FIN DE PRUEBA */