...
function addBotonSelectorSolicitud( btnId, btnText, functionOnClick = null, insertBeforeElement=null, insertAfterElement=null ){
	if( insertBeforeElement ){
		let index = arraySelectorBotonesClean.findIndex( btn => {
			return btn.id == insertBeforeElement.id
		} )
		arraySelectorBotonesClean.splice( index, 0, {id: btnId, name: btnText} )
	}else if( insertAfterElement ){
		let index = arraySelectorBotonesClean.findIndex( btn => {
			return btn.id == insertAfterElement.id
		} )
		arraySelectorBotonesClean.splice( index, 0, {id: btnId, name: btnText} )
...
function prevDefaultBotonesFormulario(){
	...
	let form_action = jQuery('[name="form_action"]');

	const cancelarBtn = jQuery("#cancelar-btn");
	if( cancelarBtn ){
	...
	const guardarBtn = jQuery("#guardar-btn");
	if ( guardarBtn ){
    guardarBtn.bindFirst("click", async function () {
      if (configuracionFormularioSolicitud.log_level >= 2) {
	...
	const enviarBtn = jQuery("#enviar-btn")
	if( enviarBtn ){
		enviarBtn.bindFirst("click",  e => {
			if (configuracionFormularioSolicitud.log_level >= 2) {
				console.log("save_button before: ");
				console.log(inSaveButton.val(''));
			}
	...
	const borrarBtn = jQuery("#borrar-btn")
	if ( borrarBtn ){
    jQuery("#borrar-btn").bindFirst("click", (e) => {
      form_action.val("remove");
    });
	}
	
	const errorBtn = jQuery("#error-btn")
	if( errorBtn ){
		errorBtn.bindFirst("click", (e) => {
			form_action.val("error_oca");
		});
	}

	const anteriorBtn = jQuery("#anterior-btn");
	if( anteriorBtn ){
		anteriorBtn.click( function(e){
			e.preventDefault();
			//console.log(e)
	...
	const siguienteBtn = jQuery('#siguiente-btn')
	if( siguienteBtn ){
...
function validateRequiredInputsSection( opcionActual ){
	...
	if( opcionActual === 'documentos-adjuntos' ){
		...
		insToValidate.forEach( i => {
			parent = findParent({ element: i, find: "grid-adjuntos-item" });
			parent.classList.remove('error')
			let tag_nombre = parent.querySelector('.tag-nombre-archivo')
			console.log({'inToValidate' : i, 'parent': parent})
			if( !i.value ){
				if( !tag_nombre || tag_nombre.innerHTML == "" ) {
          console.log('IN Vacío');
					parent.classList.add('error')
					allFieldsValidated = false
					return false;
        }else{
					console.log('Relleno de JSON')
					return true;
				}
			}
		} )
			...
			( res ) => {
				...
				res.json().then(
					( data ) => {
						...
						change_form_action_buttons( data.botones_formulario )
						prevDefaultBotonesFormulario();
...
function change_form_action_buttons( arr_buttons ){
	console.log( 'change_form_action' )
	console.log( arr_buttons )

	let button_container = document.querySelector('.contenedor-botones-formulario')
	removeChilds(button_container)

	arr_buttons.forEach( btn => {
		let el = document.createElement('button')
		el.id = `${btn}-btn`;
		switch(btn){
			case 'anterior':
				el.innerHTML = `<i class="fa-solid fa-chevron-left"></i>Anterior`;
				break;
			...
			case 'error':
				el.innerHTML = `Contiene Errores`;
				break;

			case 'siguiente':
				el.innerHTML = `Siguiente<i class="fa-solid fa-chevron-right"></i>`;
				break;
		}
		button_container.appendChild( el )
	})
}
...
async function build_selector_and_functionality(){
	if (configuracionFormularioSolicitud.log_level >= 2) {
		console.log('build_selector_and_functionality()')
  }
	await onchange_inputs_form('form.wpcf7-form');
	//prevDefaultBotonesFormulario();
	buildBotonesSelector();
	seleccionar_opcion_defecto();
}
			...
			if(["otros_documentos_adjuntos[]"].includes(input.name)){
				...
				files.forEach( async f => {
					...
					borrarBtn.addEventListener('click', async e=>{
						...
						console.log( `qs: ${qs}`)
						grid_adjuntos_item.querySelector(qs).value = 1;
						try {
...
jQuery('document').ready(()=>{
	document.addEventListener(
		...
		function (event) {
			...
			// ( jQuery('[name="save_button"]').val() ) ? window.location.href = "/home" : window.location.reload()
		},
		false
	);
	document.addEventListener(
