...
/* FUNCTION RESET FORM FILES */
function reset_form_files(){
	let to_reset = document.querySelector(
    ".seccion-documentos-adjuntos-wrapper .tag-nombre-archivo, .seccion-documentos-adjuntos-wrapper .tag-nombre-archivo,.seccion-documentos-adjuntos-wrapper .tag-nombre-archivo,"
  );
}
/* FIN FUNCTION RESET FORM FILES */

/* FUNCTION UPDATE FORM FILES */
function update_form_files(){
	console.log( window.form_files )
	Object.entries( window.form_files ).forEach( form_file => {
		console.log( form_file )

		const [inputName, file] = form_file
      ...
      `input[name*="${inputName}"]`
		...
		if (["otros_documentos_adjuntos"].includes(inputName)) {
			let parent = document.querySelector(
        `#grid-${inputName.replaceAll("_", "-")} .file-list`
      );
			removeChilds( parent )
      ...
      file.forEach((f) => {
        let html = `
					<div id="${inputName}-row-${f.identificador}" class="grid-adjuntos-item documento-adjunto-wrapper">
						<div class="grupo-formulario">
						...
						<div class="grupo-formulario">
							<button id="${inputName}-borrar-${f.identificador}" class="borrar-documento-btn"><i class="fa-regular fa-trash-can"></i></button>
						</div>
					</div>
						<input type="hidden" id="${inputName}_remove_${f.identificador}"  name="${inputName}_remove_${f.identificador}" value="0" />`;

        let html_DOM = parser.parseFromString(html, "text/html");
        console.log(`HTML:
				${html}`);
        ...
        console.log(parent);
				console.log(`querySelector("#${inputName}-borrar-${f.identificador}")`)
				document.querySelector(`#${inputName}-borrar-${f.identificador}`).addEventListener("click", (e) => {
						e.preventDefault();
						// console.log(`row: document.querySelector(#${inputName}-row-${f.identificador}`)
						// let row = document.querySelector(
						// 	`#${inputName}-row-${f.identificador}`
						// );
						// row.parentElement.removeChild(row);
						// console.log(`remove_input: document.querySelector( #${inputName}_remove_${f.identificador}
						// )`)
						if( window.form_file[inputName] ){
							delete window.form_file[inputName]
						}
						window.form_files[inputName] = window.form_files[inputName].filter( function(obj) {
							return obj.identificador !== f.identificador
						} )
						console.log("FORM FILES");
						console.log(window.form_files)
						update_form_files();
						document.querySelector(
							`#${inputName}_remove_${f.identificador}`
						).value = 1;
					});
      });
    }else{
      ...
      borrarBtn.addEventListener('click', async e=>{
      	...
      	try {
      		// grid_adjuntos_item.querySelector(".tag-nombre-archivo").innerHTML = ''
					// grid_adjuntos_item.querySelector(".tag-fecha-adjuntado").innerHTML = '';
					// grid_adjuntos_item.querySelector(".tag-estado-envio").innerHTML = "";
					// window.form_files = window.form_files.filter(function (obj) {
          //   return obj.identificador !== file.identificador;
          // });
					if ( window.form_files[inputName] ) {
            delete window.form_files[inputName];
          }
					update_form_files()
					console.log('FORM FILES')
					console.log(window.form_files)
...
/* FIN FUNCTION UPDATE FORM FILES */
	...
	switch (input.type){
		...
		case "file":
			...
			if(["otros_documentos_adjuntos[]"].includes(input.name)){
				...
				files.forEach( f => {
					window.form_files['otros_documentos_adjuntos'].push( f )
				} )
				...
				files.forEach( async f => {
					...
					if( keyExists( window.form_files, input.name ) ){
						window.form_files[input.name] = f;
					}else{
						window.form_files[input.name] = f;
					}
