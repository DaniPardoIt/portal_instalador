...
function checkNuevaSolicitudInputs(){
	...
	let arrInputs = [ inCampoActuacion, inTipoExpediente, inTipoInstalacion, inSubtipoInstalacion, inDenominacion ]
	...
	if( nuevaSolicitudOK ){
		window.location.href = `/solicitud-formulario?campo_actuacion=${inCampoActuacion.value}&tipo_expediente=${inTipoExpediente.value}&tipo_instalacion=${inTipoInstalacion.value}&subtipo_instalacion=${inSubtipoInstalacion.value}&denominacion=${inDenominacion.value}&debug=false`;
	}else{
		addNotification(
			'Error',
			msgError,
