...
jQuery('document').ready( function(){
	if( window.user && window.user.meta_data && window.user.meta_data['wp-approve-user'] ){
	...
	jQuery('[name="user_login"], [name="user_email"]').change(async function (event) {
		let input = event.target
		let grupo = jQuery(input).closest('.grupo-formulario')
		const saveButton = jQuery("#save-btn");
		...
		console.log(`fetch('/wp-json/oondeo/v1/check_user_exists?search=${input.value}')`)

    let response = await fetch(`/wp-json/oondeo/v1/check_user_exists?search=${input.value}`)
      ...
      let json = await response.json();
			...
			console.log(data)
			if( !data ){
				// La API devuelve 'false', indicando que no hay ningún usuario que coincida con esos parámetros
				return;
			}
			// La API devuelve 'true', indicando que ya existe un usuario, así que no se puede guardar.
			addNotification('El usuario ya existe', 'No puedes crear un usuario que ya existe', 'error');
			grupo.toggleClass('error')
